import React, { useState, useEffect } from 'react'; import { useAuth } from '../../contexts/AuthContext';
 ...
 proceso.estadisticas?.total_procesos_internos > 0
 );
 // Verificar si hay cambios en los procesos
 const procesosAnteriores = direcciones.map(d => d.id).sort();
 const procesosNuevos = procesosConContenido.map(d => d.id).sort();
 const hayCambios = JSON.stringify(procesosAnteriores) !== JSON.stringify(procesosNuevos);
 setDirecciones(procesosConContenido || []);
 // Si hay cambios y hay un proceso seleccionado, verificar si sigue existiendo
 ...
 console.error('Error al cargar procesos generales:', error);
 } finally {
 setLoading(false);
 }
 };
 ...
 console.error('Error al cargar procesos internos:', error);
 } finally {
 setLoading(false);
 }
 };
 // Nueva función para cargar documentos por proceso general y proceso interno con paginación
 const loadDocumentsByDirection = async (procesoGeneralId, procesoInternoId = null) => {
 try {
 setLoading(true);
 const params = new URLSearchParams({
 proceso_general_id: procesoGeneralId,
 per_page: '10' // Paginación de 10 documentos
 });
 // Si hay proceso interno seleccionado, agregarlo al filtro
 if (procesoInternoId) {
 ...
 console.error('Error al cargar documentos por proceso general/interno:', error);
 } finally {
 setLoading(false);
 }
 };
